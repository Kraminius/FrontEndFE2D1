...
function CardInputs({onValidated} : CardInputsProps) {
    ...
    const handleChange = (field: keyof CardInformationForm) => (value: string) => {
...
const validateCardForm = (formData: CardInformationForm) => {
    ...
    function isValidCardNumber(cardNumber: string): boolean { //16 digits, can have spacebar or '-' between every four numbers.
    ...
    function isValidExpiryDate(expiryDate: string): boolean {
        ...
        if (year < currentYear || (year === currentYear && month < currentMonth)) { //Has date passed
            return false;
        }
        return true;
    }
